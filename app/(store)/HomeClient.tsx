...
import { isScheduleActive } from "@/lib/utils/timeHelper";
...
export default function HomeClient({ initialCategories, initialAds, initialBestSellers, initialDiscounts = [] }: HomeClientProps) {
  ...
  const isCategoryVisible = (c: any): boolean => {
    ...
    return isScheduleActive(c.applicableDays, c.timeSlots, c.startTime, c.endTime);
  };
            ...
            {REVIEWS.map((review, i) => (
              ...
              >
                ...
                <div className="flex items-center gap-3.5 mb-4">
                  <div className="w-12 h-12 rounded-2xl bg-gradient-to-br from-primary to-[#ff4757] flex items-center justify-center text-white text-base font-black shadow-md shadow-primary/10 group-hover:scale-110 transition-transform duration-500">
                    {review.initial}
                  </div>
                  <div>
                    <h4 className="font-extrabold text-warm-900 text-sm">{review.name}</h4>
                    <span className="text-[10px] text-warm-450">{review.date}</span>
                  </div>
                </div>
                ...
                <p className="text-warm-700 text-xs sm:text-sm leading-relaxed mb-4 min-h-[4.5rem]">
                  &ldquo;{review.text}&rdquo;
                </p>

                <div className="border-t border-warm-100 pt-3.5 flex justify-between items-center text-[10px] sm:text-xs">
                  <span className="text-warm-400">Ordered</span>
                  <span className="font-bold text-warm-700 bg-warm-100 px-2 py-0.5 rounded-md">{review.orderedItem}</span>
                </div>
              </motion.div>
            ))}
          </div>
