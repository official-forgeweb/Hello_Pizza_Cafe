...
declare global {
  var redisQueueConnection: Redis | undefined;
  var redisWorkerConnection: Redis | undefined;
  var campaignQueueGlobal: Queue | undefined;
  var campaignWorkerGlobal: Worker | undefined;
}

let campaignQueue: Queue | null = null;

if (process.env.REDIS_URL) {
  // Use separate connection instances for Queue and Worker as required by BullMQ rules
  if (!globalThis.redisQueueConnection) {
    globalThis.redisQueueConnection = new Redis(process.env.REDIS_URL, {
      maxRetriesPerRequest: null,
    });
  }

  if (!globalThis.redisWorkerConnection) {
    globalThis.redisWorkerConnection = new Redis(process.env.REDIS_URL, {
      maxRetriesPerRequest: null,
    });
  }

  const queueConnection = globalThis.redisQueueConnection;
  const workerConnection = globalThis.redisWorkerConnection;

  // Initialize and cache Queue globally
  if (!globalThis.campaignQueueGlobal) {
    globalThis.campaignQueueGlobal = new Queue('whatsapp-campaign', { connection: queueConnection });
  }
  campaignQueue = globalThis.campaignQueueGlobal;

  // Initialize and cache Worker globally
  if (!globalThis.campaignWorkerGlobal) {
    globalThis.campaignWorkerGlobal = new Worker('whatsapp-campaign', async (job: Job) => {
    ...
    globalThis.campaignWorkerGlobal.on('completed', async (job) => {
    ...
    globalThis.campaignWorkerGlobal.on('failed', async (job, err) => {
